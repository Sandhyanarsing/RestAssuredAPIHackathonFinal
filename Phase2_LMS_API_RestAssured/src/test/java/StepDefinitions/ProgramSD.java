...
public class ProgramSD {
	...
	public void admin_creates_post_request_for_the_lms_with_request_bodypm()
			throws InvalidFormatException, IOException {
		data = ExcelReader.getData(URLs.ExcelPath, "ProgramRequest");
		// System.out.println(data);
	}

	@When("Admin sends HTTPS Request and  request Body with endpointpm")
	public void admin_sends_https_request_and_request_body_with_endpointpm()
			throws InvalidFormatException, IOException {

		for (int i = 0; i < data.size(); i++) {

			String requestBody = payload.ProgramData(data, i);
			response = RestAssured.given().spec(RS.createReq(URLs.createProgram)).body(requestBody).when().post();
	...
	public void admin_sends_https_request_and_request_body_with_invalid_endpointpm() {
		...
		String requestBody = payload.inavaliddata();
		System.out.println(requestBody);
		response = RestAssured.given().spec(RS.createReq(URLs.InvalidEndPoint)).body(requestBody).when().post();
		System.out.println(requestBody);
...
//		 //programName = auth.setPMProgramName();
	...
	public void admin_sends_https_request_with_invalid_endpointpm() {
		...
		response = RestAssured.given().spec(RS.createReq(URLs.InvalidEndPoint)).when().get();
	...
	public void admin_sends_https_request_with_invalid_programidpm() {
		// programID =auth.setPMProgramID();
		// programName = auth.setPMProgramName();
		response = RestAssured.given().spec(RS.createReq(URLs.Invalidprogramid)).when().get();
	...
	public void admin_sends_https_request_with_valid_enpointpm() {
		...
		response = RestAssured.given().spec(RS.createReq(URLs.GetAllPrograms)).when().get();
	...
	public void admin_sends_https_request_with_invalid_base_urlspm() {
		...
		response = RestAssured.given()
				// .header("Authorization", "Bearer "+ auth.setAuthorisation())
				.spec(RS.createReqwithInvalidBaseurl(URLs.GetProgramByProgramId))
				.pathParam("programId", Authorization.scenarioContext.getContext(LMSConstants.PROGRAMID_KEY))
				// .body(requestBody)
				.when().get();
	...
	public void admin_sends_https_request_with_invalid_baseurlpm() {
		...
		response = RestAssured.given()
				// .header("Authorization", "Bearer "+ auth.setAuthorisation())
				.spec(RS.createReqwithInvalidBaseurl(URLs.GetAllPrograms)).when().get();
	}
	...
	public void admin_creates_put_request_for_the_lms_api_endpoint_with_valid_request_body_with_mandatory_additional_fieldspm()
			throws InvalidFormatException, IOException {
		data = ExcelReader.getData(URLs.ExcelPath, "ProgramRequest");
	}
	...
	public void admin_sends_https_request_with_valid_endpoint_updatepm() throws InvalidFormatException, IOException {
		for (int i = 0; i < data.size(); i++) {
			...
			response = RestAssured.given()
					...
					.pathParam("programName",
							Authorization.scenarioContext.getContext(LMSConstants.PROGRAMNAME_KEY, programName))
					.body(requestBody).when().put();
	...
	public void admin_sends_https_request_with_invalid_method_using_programnamepm() {
		...
		String requestBody = payload.inavaliddata();
		response = RestAssured.given().spec(RS.createReq(URLs.UpdateProgramByProgramName)).body(requestBody)
				.pathParam("programName", Authorization.scenarioContext.getContext(LMSConstants.PROGRAMNAME_KEY)).when()
				.get();
	}
	...
	public void admin_sends_https_request_with_invalid_endpoint_updatepm() {
		...
		response = RestAssured.given()
				// .header("Authorization", "Bearer "+ auth.setAuthorisation())
				.spec(RS.createReq(URLs.InvalidEndPoint)).body(requestBody).when().put();
		System.out.println(requestBody);
	...
	public void admin_creates_put_request_for_the_lms_api_endpoint_with_valid_request_body_with_programidpm()
			throws InvalidFormatException, IOException {
		data = ExcelReader.getData(URLs.ExcelPath, "ProgramRequest");
	...
	public void admin_sends_https_request_with_endpoint_update_programidpm()
			throws InvalidFormatException, IOException {
		// programId =auth.setPMProgramID();
		for (int i = 0; i < data.size(); i++) {
			String requestBody = payload.UpdateProgramData(data, i);
	...
	public void admin_sends_https_request_with_invalid_base_urlpm() {
		...
		response = RestAssured.given()
				// .header("Authorization", "Bearer "+ auth.setAuthorisation())
				.spec(RS.createReqwithInvalidBaseurl(URLs.UpdateProgramByProgramId)).body(requestBody)
				.pathParam("programId", Authorization.scenarioContext.getContext(LMSConstants.PROGRAMID_KEY)).when()
				.put();

	}

	@Then("Then Admin receives Status with message and boolean success details with invalid base urlpm")
